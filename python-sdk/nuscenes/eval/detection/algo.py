...
def custom_accumulate(gt_boxes: EvalBoxes,
    ...
    conf = np.mean(np.array(conf)).astype(float)
    ...
    # Done. Instantiate MetricData and return
    # ---------------------------------------------
    DetectionMetricData.nelem = 2
    return DetectionMetricData(recall=np.tile(rec, (2, 1)),
                               precision=np.tile(prec, (2, 1)),
                               confidence=np.tile(conf, (2, 1)),
                               trans_err=np.tile(match_data['trans_err'], (2, 1)),
                               vel_err=np.tile(match_data['vel_err'], (2, 1)),
                               scale_err=np.tile(match_data['scale_err'], (2, 1)),
                               orient_err=np.tile(match_data['orient_err'], (2, 1)),
                               attr_err=np.tile(match_data['attr_err'], (2, 1)))
...
def calc_md(md: DetectionMetricData, min_recall: float, min_precision: float) -> float:
    ...
    md_ = np.copy(md)
    md_ = md_[round(100 * min_recall) + 1:]  # Clip low recalls. +1 to exclude the min recall bin.
    md_ -= min_recall  # Clip low precision
    md_[md_ < 0] = 0
    return float(np.mean(md_)) / (1.0 - min_precision)
